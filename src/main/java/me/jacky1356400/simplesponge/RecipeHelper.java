...
public class RecipeHelper {
    ...
    public static IRecipe addRecipe(int j, IRecipe rec){
        CraftingManager.func_193372_a(new ResourceLocation(SimpleSponge.MODID, "recipes" + j), rec);
        return rec;
    }
    /*
     * Adds a shapeless recipes with X Output using a list of inputs.
     */
    public static IRecipe addShapeless(ItemStack output, List<ItemStack> inputs){
        return addShapeless(output, inputs.toArray());
    }
    ...
    public static IRecipe addShapeless(ItemStack output, Object[] inputs){
        return addRecipe(j++, new ShapelessRecipes(String.valueOf(j), output, createInput(inputs)));
    }
    ...
    public static IRecipe addShapeless(Item output, List<ItemStack> inputs){
        return addShapeless(output, inputs.toArray());
    }

    public static IRecipe addShapeless(Item output, Object[] inputs){
        return addRecipe(j++, new ShapelessRecipes(String.valueOf(j), new ItemStack(output), createInput(inputs)));
    }

    public static IRecipe addShaped(Item output, int width, int height, Object[] input) {
        return addRecipe(j++, genShaped(new ItemStack(output), width, height, input));
    }

    public static IRecipe addShapeless(Block output, List<ItemStack> inputs){
        return addShapeless(output, inputs.toArray());
    }

    public static IRecipe addShapeless(Block output, Object[] inputs){
        return addRecipe(j++, new ShapelessRecipes(String.valueOf(j), new ItemStack(output), createInput(inputs)));
    }

    public static IRecipe addShaped(Block output, int width, int height, Object[] input) {
        return addRecipe(j++, genShaped(new ItemStack(output), width, height, input));
    }
    ...
    public static NonNullList<Ingredient> createInput(Object[] input){
        ...
        for(int i = 0; i < input.length; i++){
            ...
            if(k instanceof String){
                ...
                throw new UnsupportedOperationException("Attempted to add invalid shapeless recipe. Please contact the author of Simple Sponge.");
            }
        }
        return inputL;
    }
